...
class CorpusBuilder:
    def __init__(self, inputPath, corpusSize=0):
        ...
        # self.corpus = ' '.join(list(pandas.read_csv(inputPath)['NEWS_BODY']))
        corpustext = clean_str(self.corpus)
...
# 어절 리스트 구축 (inputToFormat, clean_str, longestSub, 클래스에 의존)
def eoListBuilder(input, index):
    # 고려사항: 이 때 Mecab을 사용해서 조사를 모두 제거하나?
    # 명사추출 후 TF-IDF를 사용?
    # 명사추출하면서 RP를 모두 리스트로 뽑아보나?
    ...
    # print(longestSub('1인', eoL))

    # 말뭉치 어절 리스트
    out = []
    for i in eoL: # 이 eoList는 문서 단위의 eoList
        # print(longestSub(i, corpusEoList)) # 이 eoList는 corpusEoList를 사용하는것이 맞음
        out.append(longestSub(i, cb.corpusEoList))
    out = list(filter(None, out))
    out = list(dict.fromkeys(out))
    return out

# 마지막 형태소가 조사일 경우 제거 (클래스에 의존)
# 끝이 조사인 경우 boolean으로 출력하는 함수가 필요할지?
def removeJ(eo):
    split = cb.m.pos(eo)
    for i in cb.exclude:
...
# TF-IDF calculation (totalDocs에 의존)
def calcTFIDF(text, doc, corpus):
    # calculate TF
    tf = doc.count(text)
    if tf == 0: tf = 0.001
    # calculate IDF
    docList = list(pandas.read_csv(corpus)['NEWS_BODY'])
    deno = 0
    for i in docList:
        if text in i:
            deno += 1
    if deno == 0: deno = 0.001
    idf = math.log(totalDocs(corpus) / deno)
    print(tf*idf)

# corpusDocList = list(pandas.read_csv(r"C:/comfinder/longtext.csv")['NEWS_BODY'])
# completeCorpus = ' '.join(corpusDocList)
...
for i in range(10):
    ...
    for j in extractList(eoL, 2):
        temp.append(removeJ(j))
    print(temp)


# 조사를 제거한 명사 출력
# print(inputToFormat(input, 7))
# removeJ("셀트리온")
# removeJ("순호감도")
