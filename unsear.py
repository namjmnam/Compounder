...
for i in range(iterNum):
    corpus += inputToFormat(inputPath, i)
corpus = clean_str(corpus)
temp = corpus.split(' ')
corpusEoList = [i for i in temp if len(re.sub(r'[0-9]+', '-', i)) >= 2]

# 말뭉치 어절 리스트
out = []
for i in eoL: # 이 eoList는 문서 단위의 eoList
    # print(longestSub(i, corpusEoList)) # 이 eoList는 corpusEoList를 사용하는것이 맞음
    out.append(longestSub(i, corpusEoList))
out = list(filter(None, out))
out = list(dict.fromkeys(out))
# print(out)

exclude = ['VV', 'VA', 'VX', 'VCP', 'VCN', 'MM', 'MAG', 'MAJ', 'IC', 'JKS', 'JKC', 'JKG', 'JKO', 'JKB', 'JKV', 'JKQ', 'JX', 'JC', 'EP', 'EF', 'EC', 'ETM', 'XPN', 'XSV', 'XSA', 'XR']
temp = []
for i in out: # e.g '설명했다'
    possiblyNoun = True
    for j in m.pos(i): # ('설명', 'NNG') in [('설명', 'NNG'), ('했', 'XSV+EP'), ('다', 'EC')]
        # if len(m.pos(i)) == 1 and m.pos(i)[0][1][0] == 'N':
        #     # 이미 등록된 명사일 경우
        #     possiblyNoun = False
        #     break
        for k in exclude:
            if k in j[1] or j == ('의', 'NNG'):
                # 끝에 조사가 오거나 '의'로 끝날 경우
                possiblyNoun = False
