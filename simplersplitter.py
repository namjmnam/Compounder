    ...
    def __init__(self, inputText, inputCorpus=None):
        ...
        for i in self.lplist:
            ...
            for j in range(len(i)):
                # 현재는 단순히 대상에 띄어쓰기+단어가 검색된 갯수만 찾지만 본래 어절의 좌측부분만 검색하도록 해야 함
        ...
        for i in range(len(self.fList)):
            ...
            for j in n:
                ...
                if (self.searchSpaceless(j, self.rawdoc) > 1 or self.searchSpaceless(j, self.parenthesisButer(self.rawdoc)) > 1)\
                    and j not in self.allCW:
                    self.allCW.append(j)

        # Mecab을 사용하여 기존에 등록되어있는 명사 제외 (가장 마지막에 실행되어야 함)
        temp = self.extnouns[:]
        for i in temp:
            if len(m.pos(i)) == 1 and m.pos(i)[0][1][0] == 'N':
                self.extnouns.remove(i)

    # compounder에서 사용하는 복합어일 가능성이 있는 wordpair 추출 방식
...
# print(s.trpmidict)
# print(s.detectConnection(s.pairList))

# 기존 방식
# print(s.fList)
print(s.allCW)
# print(s.genCW(s.fList[0]))
