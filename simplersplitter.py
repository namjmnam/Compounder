...
class Splitter:
    def __init__(self, inputText, inputCorpus=None):
        ...
        if inputCorpus == None: self.corpus = self.rawdoc
        else: self.corpus = self.trim_str(' ' + inputCorpus)
        ...
        # 어절 리스트 생성
        temp = self.doc.split(' ')
        self.eoList = [i for i in temp if len(re.sub(r'[0-9]+', '-', i)) >= 3]
        ...
        for i in self.lplist:
            ...
            for j in range(len(i)):
                ...
                # 말뭉치 내 단어 수 산출 (미사용)
                scores.append(self.corpus.count(i[j])) # (TBF)

                # 아이디어: LP를 검색해서 scores에 등록할 때 다음 글자가 더 긴 LP의 다음 글자인 경우를 제외?
                # 예: LP = 마스턴투자 -> 마스턴투자운(카운트 포함X) 마스턴투자자(카운트 포함O)
                # 이런 식으로 검색해서 마스턴투자운의 빈도가 마스턴투자의 빈도보다 현저히 높은 경우 마스턴투자를 discard
            ...
            #         finalscore = scores[j]
            #         break
            #     # 예: 마스터투자운 -> 마스턴투자운용 빈도수가 크게 차이가 안 날 경우 넘어가지만
            #     # 마스터투자운용 -> 마스턴투자운용은 빈도수가 크게 차이가 나기 때문에 그 직전에 명사로 채택
            ...
            for j in range(len(chosen)):
                # if finalscore[j] >= 2: self.extnouns.append(chosen[j]) # 이 인자는 차후 수정 가능
                # score가 말뭉치에 대해서 검색한 경우 (미사용)
                if self.rawdoc.count(chosen[j]) >= 2: self.extnouns.append(chosen[j]) # (TBF)
        ...
        for i in self.extnouns:
            for j in self.extnouns:
                # rawdoc으로 검색할 수도 있고 corpus로 검색할 수도 있음 (기존의 2번과 달리 1번 이상 등장하는 경우 등록) (TBF: 현재 테스트용으로 1을 2로 고침)
                if self.searchSpaceless([i, j], self.rawdoc) >= 2 or self.searchSpaceless([i, j], self.parenthesisBuster(self.rawdoc)) >= 2\
        ...
        for i in range(len(self.fList)):
            ...
            for j in n:
                ...
                # rawdoc에 대해서 검색중이나 차후 corpus 사용 여부 고려 중
                if (self.searchSpaceless(j, self.rawdoc) >= 2 or self.searchSpaceless(j, self.parenthesisBuster(self.rawdoc)) >= 2)\
...
# # corpus = ' '.join(list(pandas.read_csv(input, encoding='utf8')['NEWS_BODY']))
# # CSV파일의 행 수 만큼 코드 실행
# # out = []
...
# # df = df[['URL_CD', 'NEWS_BODY']].loc[0:49]
# # for i in range(50):
# #     str = df.at[i, 'NEWS_BODY']
# #     str = str.replace('\n', ' ').replace('\t', ' ').replace('\r', ' ').replace('"', ' ').upper()
# #     str = re.sub(' +', ' ', str)
# #     df.at[i, 'NEWS_BODY'] = str
# df['EN_LIST'] = [''] * 50
# # df.to_csv(r"C:/comfinder/input.csv", encoding='utf8', index=False)
...
# df.to_csv(r"C:/comfinder/output.csv", encoding='euc-kr', index=False)
...
corpus = ' '.join(list(pandas.read_csv(inputPath, encoding='utf8')['NEWS_BODY']))
iterNum = totalDocs(inputPath)
if iterNum > 30: iterNum = 30
for i in range(iterNum):
    out = []
    text = inputToFormat(inputPath, i)
    s = Splitter(text, corpus)
    out += [i for i in list(s.trpmidictnew.keys()) if s.trpmidictnew[i] >= 0.3] # 복합단어 추출 방식1
